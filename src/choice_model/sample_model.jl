...
function sample_choices_all_trials(data::Dict, pz::Vector{Float64}, pd::Vector{Float64}; 
    ...
    Random.seed!(rng)
    nT,nL,nR = bin_clicks(data["T"],data["left"],data["right"]; dt=dtMC, use_bin_center=use_bin_center)
    
    choices = pmap((nT,L,R,nL,nR,rng) -> sample_choice_single_trial(nT,L,R,nL,nR,pz,pd;
            use_bin_center=use_bin_center, rng=rng), nT, data["left"], data["right"], nL, nR, shuffle(1:length(data["T"])))
