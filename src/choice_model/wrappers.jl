...
function default_parameters(;generative::Bool=false)
    ...
    pz = Dict("name" => ["σ_i","B","B_λ", "λ", "σ_a","σ_s","ϕ","τ_ϕ","η","α_prior","β_prior","B_0","γ_shapeL", "γ_scaleL", "γ_shapeR", "γ_scaleR"],
              "fit" => vcat(false, true, true, false, false, true, false, false, false, false, false, false, false, false, false, false),
              "initial" => [eps(), 0.7, rand(), -0.001, eps(), 2. + rand()* (4. - 2.), 0.15, 0.02, rand(), rand(), rand() * 2., rand(), rand(), rand(), rand(), rand()],
              "lb" => [0., 0.6, eps(), -5., 0., 0., 0.01, 0.005, 0., 0., 0., -15., 0., 0., 0., 0.],
              "ub" => [2., 1., 15., 5., 20., 20., 1.2, 1., 1., 1., 30., 15., 5., 5., 5., 5.])

    if generative
        pz["generative"] = [eps(), 0.7, 1, -eps(), eps(), 2., 0.15, 0.02, 0.5, 0.7, 4., 0., 3., 0.03, 3., 0.03]
        pd["generative"] = [0.,0.0]
