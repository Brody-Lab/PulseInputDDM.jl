...
function predict_in_sample(model::DDLM)
    ...
    β = map((βcoupled, coupling, trialset)->map((βcoupled, coupling, unit)->βcoupled.*coupling .+ unit.βuncoupled.*(1-coupling), βcoupled, coupling, trialset.units), βcoupled, θ.coupling, data)
