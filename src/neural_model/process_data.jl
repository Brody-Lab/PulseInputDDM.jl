...
function aggregate_spiking_data(path::String, sessids::Vector{Vector{Int}}, ratnames::Vector{String};
    ...
    data = vcat(map((rat,data,sess)-> map((d,s)-> 
            neural_data_single_session(d, rat, s; break_session=break_session), data, sess), ratnames, rawdata, sessids)...)
            
end

function neural_data_single_session(rawdata, ratname, sessID; break_session::Bool=false)
    ...
    if break_session
        ...
        for n = 1:N
            ...
            data[n]["ntrials"] = ntrials
            data[n]["N"] = 1
            data[n]["synthetic"] = false

            data[n]["T"] = vec(rawdata["T"])
            data[n]["pokedR"] = vec(convert(BitArray,rawdata["pokedR"]))
            data[n]["correct_dir"] = vec(convert(BitArray,rawdata["correct_dir"]))

            data[n]["leftbups"] = map(x-> vec(collect(x)), rawdata["leftbups"])
            data[n]["rightbups"] = map(x-> vec(collect(x)), rawdata["rightbups"])
            data[n]["sessID"] = repeat([sessID], inner=ntrials)
            data[n]["ratID"] = repeat([ratname], inner=ntrials)

            data[n]["stim_start"] = rawdata["stim_start"]
            data[n]["cpoke_end"] = rawdata["cpoke_end"]
            data[n]["cellID"] = vec(map(x-> Int.(vec(collect([x[n]]))), rawdata["cellID"]))
            data[n]["spike_times"] = vec(map(x-> vec(collect([x[n]])), rawdata["spike_times"]))
        ...
        data["leftbups"] = map(x-> vec(collect(x)), rawdata["leftbups"])
        data["rightbups"] = map(x-> vec(collect(x)), rawdata["rightbups"])
        data["sessID"] = repeat([sessID], inner=ntrials)
        data["ratID"] = repeat([ratname], inner=ntrials)

        data["stim_start"] = rawdata["stim_start"]
        data["cpoke_end"] = rawdata["cpoke_end"]
        data["cellID"] = vec(map(x-> Int.(vec(collect(x))), rawdata["cellID"]))
        data["spike_times"] = vec(map(x-> vec(map(y-> vec(collect(y)), x)), rawdata["spike_times"]))
...
function bin_clicks_spikes_and_λ0!(data::Dict,use_bin_center::Bool; dt::Float64=1e-2, delay::Float64=0.,
    nconds::Int=4)
    ...
    data["pad"] = 10 #25 ms before and after trial
    ...
    if data["synthetic"]
        ...
        #data["spike_counts_padded"] =  map(x -> map(z -> vcat(zeros(pad),x[z],zeros(pad)), 1:data["N"]), data["spike_counts"])
        
        #data["spike_counts_padded"] =  map((x,y) -> map(z -> vcat(poisson_noise!.(y[z][1]*ones(pad),
        #                data["dt"]),x[z], poisson_noise!.(y[z][end]*ones(pad),
        #                data["dt"])), 1:data["N"]), 
        #        data["spike_counts"],data["λ0"])
    ...
    data = process_spike_data!(data;nconds=nconds)
        
    #synthetic data keep lambda0, which might be generated comletely, or be interited from data used to generate parameters initially
    if !data["synthetic"]
