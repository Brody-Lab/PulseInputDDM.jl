...
function load_and_optimize_new(data, N; dt::Float64=1e-2, n::Int=53,
        fit_latent::BitArray{1}=trues(dimz), dimy::Int=4, show_trace::Bool=true,
    ...
    py[:initial] = optimize_model_new(dt,data,py[:fit],show_trace=false,λ0=λ0["by_neuron"],f_str=f_str)
    
    pz[:final],py[:final],opt_output, state = optimize_model_new(pz[:initial],py[:initial],
...
function do_p0(dt::Float64,data::Dict;f_str::String="softplus")
    ...
    map((trials,k)->compute_p0(ΔLR[trials],k,dt;f_str=f_str),data["trial"], data["spike_counts_by_neuron"])
...
function optimize_model_new(dt::Float64,data::Dict,fit_vec::Union{Vector{BitArray{1}},Vector{Vector{Bool}}};
        beta::Vector{Vector{Float64}}=Vector{Vector{Float64}}(),
        mu0::Vector{Vector{Float64}}=Vector{Vector{Float64}}(),
        f_str::String="softplus",
        x_tol::Float64=1e-16,f_tol::Float64=1e-16,g_tol::Float64=1e-12,
        ...
        p = map((p,trials,k,fit_vec,λ0)-> optimize_model_new(p,dt,ΔLR[trials],k,fit_vec;
            show_trace=show_trace, f_str=f_str, λ0=λ0), p, data["trial"], data["spike_counts_by_neuron"], fit_vec, λ0)
...
function optimize_model_new(p::Vector{Float64},dt::Float64,ΔLR::Vector{Vector{Int}},
        k::Vector{Vector{Int}},fit_vec::Union{BitArray{1},Vector{Bool}};
        beta::Vector{Float64}=Vector{Float64}(),
        mu0::Vector{Float64}=Vector{Float64}(),f_str::String="softplus",
        x_tol::Float64=1e-16,f_tol::Float64=1e-16,g_tol::Float64=1e-12,iterations::Int=Int(5e3),
        ...
        ###########################################################################################
        ## Optimize    
        ll(p_opt) = ll_wrapper_new(p_opt, p_const, fit_vec, k, ΔLR, dt; beta=beta, mu0=mu0, f_str=f_str, λ0=λ0)
        opt_output, state = opt_ll(p_opt,ll;g_tol=g_tol,x_tol=x_tol,f_tol=f_tol,iterations=iterations,show_trace=show_trace)
        p_opt = Optim.minimizer(opt_output)
...
function ll_wrapper_new(p_opt::Vector{TT}, p_const::Vector{Float64}, fit_vec::Union{BitArray{1},Vector{Bool}},
        k::Vector{Vector{Int}}, ΔLR::Vector{Vector{Int}}, dt::Float64;
        beta::Vector{Float64}=Vector{Float64}(0),
        mu0::Vector{Float64}=Vector{Float64}(0),
        f_str::String="softplus",
        ...
        LL = compute_LL_new(py, ΔLR, k; dt=dt, f_str=f_str, beta=beta, mu0=mu0, λ0=λ0)
    
        return -LL
            
end

function compute_LL_new(py::Vector{T}, ΔLR::Vector{Vector{Int}}, k::Vector{Vector{Int}};
        dt::Float64=1e-2, f_str="softplus",
        beta::Vector{Float64}=Vector{Float64}(),
        mu0::Vector{Float64}=Vector{Float64}(),
        λ0::Vector{Vector{Float64}}=Vector{Vector{Float64}}()) where {T <: Any}
...
function optimize_model_new(pz::Vector{TT},py::Vector{Vector{TT}},pz_fit,py_fit,data;
        dt::Float64=1e-2, n::Int=53, f_str="sig",map_str::String="exp",
        beta::Vector{Vector{Float64}}=Vector{Vector{Float64}}(),
        mu0::Vector{Vector{Float64}}=Vector{Vector{Float64}}(),
        x_tol::Float64=1e-16,f_tol::Float64=1e-16,g_tol::Float64=1e-12,
    ...
    ###########################################################################################
    ## Optimize
    ll(x) = ll_wrapper_new(x, p_const, fit_vec, data, λ0, f_str, N; dt=dt, n=n, beta=beta, 
        mu0=mu0, map_str=map_str, dimy=dimy)
    opt_output, state = opt_ll(p_opt,ll;g_tol=g_tol, x_tol=x_tol, f_tol=f_tol,
        iterations=iterations, show_trace=show_trace);
    p_opt = Optim.minimizer(opt_output)
...
function ll_wrapper_new(p_opt::Vector{TT}, p_const::Vector{Float64}, fit_vec::Union{BitArray{1},Vector{Bool}}, 
    ...
    LL = compute_LL_new(pz, py, data, dt=dt, n=n, f_str=f_str, λ0=λ0, beta=beta, mu0 = mu0)
    
    return -LL
              
end

function compute_LL_new(pz::Vector{T},py::Vector{Vector{T}},data;
        dt::Float64=1e-2, n::Int=53,f_str="softplus",
        beta::Vector{Vector{Float64}}=Vector{Vector{Float64}}(),
        mu0::Vector{Vector{Float64}}=Vector{Vector{Float64}}(),
        λ0::Vector{Vector{Vector{Float64}}}=Vector{Vector{Vector{Float64}}}()) where {T <: Any}
    
    LL = sum(LL_all_trials_new(pz, py, data, dt=dt, n=n, f_str=f_str, λ0=λ0))
