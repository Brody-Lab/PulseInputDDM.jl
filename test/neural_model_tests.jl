...
@test round(loglikelihood(model_gen), digits=2) ≈ -319.64

@test round(norm(gradient(model_gen)), digits=2) ≈ 32.68

x = PulseInputDDM.flatten(θ)
@test round(loglikelihood(x, model_gen), digits=2) ≈ -319.64
...
@test round(loglikelihood(model0), digits=2) ≈ -1495.92

x0 = PulseInputDDM.flatten(θ0)
@unpack f = θ0

@test round(loglikelihood(x0, model0), digits=2) ≈ -1495.92
...
@test round(norm(gradient(model)), digits=2) ≈ 260.59
