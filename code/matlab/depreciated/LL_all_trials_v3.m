function varargout = LL_all_trials_v3(xv,data,dt,n,dimz,dimd,dimy,use,...
    N,fr_func,posterior,xs,fit_vec)
...
%        11: fr_func: function handle for the tuning curve
%        12: posterior: flag to compute or not compute the posterior over
%           the latent
%        13: xs: other model parameters that are not being optimized
%        14: fit_vec: binary vector of which parameters are being fit and
%           which are not
...
%set the initial variance using
%the "settling" method of Brunton et. al.
...
if posterior
    ...
    parfor i = 1:tri
        [post{i},alpha{i}] = LL_single_trial(data(i),x(6:8),P,M,dx,xc,...
            Sfrac,nbinsL,lambda(:,data(i).N),n,dt,use,posterior);
    end
...
%        12: use: struct stating which types of data to use
