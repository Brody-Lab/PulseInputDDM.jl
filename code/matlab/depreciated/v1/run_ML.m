function [xf,history,x0,fit_vec,data] = run_ML(ratname,model_type,sessid,in_pth,save_pth)
...
%%
    ...
    fprintf('now run with settling only, to check that we are in fact at a min\n');
    settle = true;
    fobj = @(x) LL_all_trials(x,data,dt,n,nstdc,dimz,dimd,dimy,...
        use,N,settle,fr_func,[],x0(~fit_vec),fit_vec);
    tic; xf = fmincon(fobj,xf,[],[],[],[],lb,ub,[],options); toc;     %run optimization
